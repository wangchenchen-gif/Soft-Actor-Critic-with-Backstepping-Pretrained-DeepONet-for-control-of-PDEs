...
def myReshapeFunc2(all_data, N1=102):  # 56中每个通道22点，总输入44维
...
spatial1 = np.linspace(0, 1, 102, dtype=np.float32)
...
class BranchNet(nn.Module):
    def __init__(self, shape):
        ...
        self.fc1 = torch.nn.Linear(64 * 23, 102)
        
        self.relu = nn.ReLU()
  
    def forward(self, x):
        # 输入x的形状为 (batch_size, 44)，重塑为 (batch_size, 2, 22) 以适配Conv1d的输入 (N, C, L)
...
class CustomFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=102):
        ...
        N1 = 102
        current_dir = Path(__file__).parent.resolve()
        model_path = current_dir / "modedlNet.pt"
        
        # 创建模型
        self.net1 = dde.nn.DeepONetCartesianProd([N1, BranchNet(N1)], [1, 32,64, 102], "relu", "Glorot normal").cuda()
