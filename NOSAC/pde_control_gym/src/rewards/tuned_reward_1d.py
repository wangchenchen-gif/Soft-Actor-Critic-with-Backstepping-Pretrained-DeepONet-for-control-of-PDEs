...
class TunedReward1D(BaseReward):
    ...
    def reward(self, uVec: np.ndarray =None, time_index: int = None, terminate: Optional[bool] =None, truncate: Optional[bool] =None, action: Optional[float] =None, control_sample_rate: Optional[float]=0.01):
        ...
        if terminate and np.linalg.norm(uVec[time_index]) < 20:
            return float(self.terminate_reward - np.sum(abs(uVec[:, -1]))/1000 - np.linalg.norm(uVec[time_index]))
        if truncate:
            return self.truncate_penalty*(self.nt-time_index)
        return float(np.linalg.norm(uVec[time_index-int(1/control_sample_rate)])-np.linalg.norm(uVec[time_index]))