...
class CustomFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=101):
        ...
        # 分别为lam和u创建DeepONet
        self.net_lam = dde.nn.DeepONetCartesianProd([101, 256], [1,128,256], "relu", "Glorot normal").cuda()
        self.net_u = dde.nn.DeepONetCartesianProd([101, 256], [1,128,256], "relu", "Glorot normal").cuda()
        
        # 合并特征后的处理层
        self.fc = nn.Sequential(
      
            nn.Linear(202,128),  # 假设每个DeepONet输出256个特征
            nn.ReLU(),
    ...
    def forward(self, observations):
        # 分离lam和u
        lam = observations[:, :101]  # 假设前202个值是lam
        u = observations[:, 101:]    # 假设后202个值是u
